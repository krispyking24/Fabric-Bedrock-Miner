...
public class TaskBlockInfo {
     ...
     * @param facing 方块的朝向
     */
    public TaskBlockInfo(Block block, BlockPos pos, @Nullable Direction facing, ClientWorld world) {
        this.block = block;
        this.pos = pos;
        this.world = world;
        this.facing = facing;
    ...
    public TaskBlockInfo(Block block, BlockPos pos, ClientWorld world) {
     ...
     * 目标方块
     */
    public static class TargetBlock extends TaskBlockInfo {
    ...
    public String getBlockName() {
        return BlockUtils.getBlockName(block);
    }

    public String getIdentifier() {
        return BlockUtils.getId(block);
    }

    public Identifier getBlockId() {
        return BlockUtils.getIdentifier(block);
    }
     ...
     * 活塞
     */
    public static class Piston extends TaskBlockInfo {
        protected final boolean sticky;
        public final TargetBlock head = new TargetBlock(Blocks.PISTON_HEAD, pos.offset(facing), world);
        public Piston(boolean sticky, BlockPos pos, Direction facing, ClientWorld world) {
        ...
        public Piston(boolean sticky, BlockPos pos, ClientWorld world) {
            this(sticky, pos, Direction.UP, world);
        }

        public Piston(BlockPos pos, Direction direction, ClientWorld world) {
            this(false, pos, direction, world);
        }

        public Piston(BlockPos pos, ClientWorld world) {
            this(false, pos, Direction.UP, world);
        }

        public boolean isExtended() {
            BlockState blockState = getBlockState();
            if (blockState.contains(Properties.EXTENDED)) {
                return blockState.get(Properties.EXTENDED);
            }
            return false;
        }
     ...
     * 红石火把
     */
    public static class RedstoneTorch extends TaskBlockInfo {
     ...
     * 基座方块（建议使用粘液块）
     */
    public static class BaseBlock extends TaskBlockInfo {
